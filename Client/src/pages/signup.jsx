...
class SignupPage extends Component {
    ...
    usernameMatched = (response, registered) => {
    ...
    processResponse = (response, registered) => {
        console.log("response.data: ", response.data)
        if (response.data && response.data.userName === registered.userName) {
            this.usernameMatched(response,registered);
        }
        else {
            this.usernameUnmatched();
        }
    }

    containsBlank = (signup, login) => {
        if (signup)
            return this.state.userName === '' || this.state.phoneNumber === '' || this.state.password === '';
        else if (login)
    ...
    onSubmit = (event) => {
        event.preventDefault()
        if (this.containsBlank(this.state.is_signup, this.state.is_login)) {
            this.setState({ 
                alert : true,
        ...
        axios.post("http://localhost:4000/app/usercheck", registered)
            .then(response => {
                if (response) {
                    this.processResponse(response, registered);
                }
                this.setState({
                    userName: '',
                    phoneNumber: '',
        ...
        return (
            <div>
                ...
                <div className="heading">
                    <p className='h1 text-center'> Bruin Market </p>
                    <p className='h5 text-center'> Make the most out of your used goods! </p>
                </div>
